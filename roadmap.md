...
- Header bleibt (fuer Tab-Wechsel), Fullscreen-Exit-Button sichtbar
...
- CSS-Variablen werden per Theme-Klasse ueberschrieben
...
- Backend: `/proc/net/tcp` parsen + Docker-Socket fuer Container-Zuordnung
- API: `GET /api/ports` (gecacht, 5s)
- Frontend: Panel (aehnlich SFTP) mit Tabelle, Sortierung, Suchfilter
...
- **User** — Gleiche Moeglichkeiten wie Admin (Sessions teilen), aber keine User-/Verbindungs-Verwaltung
...
| Eigenes Passwort aendern | ja | ja | ja |
| Theme aendern | ja | ja | ja |
...
- Frontend: Rolle via `/api/auth/check` abfragen, UI entsprechend einschraenken
...
```
Phase 1: alle unabhaengig, parallel machbar
Phase 2: nach Phase 1 (damit Fullscreen-Button mitgestylt wird)
Phase 3: nach Phase 2 (Dashboard gleich theme-faehig)
